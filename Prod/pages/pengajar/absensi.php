...
<html>
    ...
    <script>
        let absensiTable = null;
        let currentJadwalId = null;

        $(document).ready(function() {
            absensiTable = new DataTable('#absensiTb', { 
                scrollX: true,
                order: [[0, 'desc']] // Default sort by tanggal descending (terbaru)
            });
            
            // Show alert if parameter success exists
            const urlParams = new URLSearchParams(window.location.search);
            if(urlParams.get('success') === '1') {
                alert('Absensi berhasil disimpan!');
                // Clean URL
                window.history.replaceState({}, document.title, window.location.pathname + '?page=absensi');
            }
        ...
        function applyFilters() {
            ...
            if (absensiTable) {
                ...
                absensiTable.order([0, sortOrder]).draw();
            }
        }
