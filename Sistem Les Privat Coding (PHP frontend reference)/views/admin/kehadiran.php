<div class="flex flex-col gap-6">
    ...
    <div class="bg-white rounded-lg shadow-md border border-gray-200">
        ...
        <div class="px-6 py-6">
            <table id="tableKehadiranAdmin" class="display w-full text-sm text-left rtl:text-right text-gray-500">
...
function applyFilters() {
    if (!tableKehadiranAdmin) return;
    tableKehadiranAdmin.draw();
}

document.addEventListener('DOMContentLoaded', () => {
    // Custom filter (Periode + Status)
    $.fn.dataTable.ext.search.push((settings, data, dataIndex) => {
        if (!settings?.nTable || settings.nTable.id !== 'tableKehadiranAdmin') return true;
        if (!tableKehadiranAdmin) return true;

        const row = tableKehadiranAdmin.row(dataIndex).data();
        if (!row) return true;
    ...
    tableKehadiranAdmin = $('#tableKehadiranAdmin').DataTable({
        data: kehadiranAdminData,
        columns: [
            {
                data: null,
    ...
    const wrapper = document.getElementById('tableKehadiranAdmin_wrapper');
